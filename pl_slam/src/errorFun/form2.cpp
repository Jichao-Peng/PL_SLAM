...
void StereoFrameHandler::gaussNewtonOptimization(Matrix4d &DT, Matrix6d &DT_cov, double &err_, int max_iters)
{
    ...
    for( iters = 0; iters < max_iters; iters++)
    {
        ...
        if (err > err_prev) {
            if (iters > 0)
                break;
            cout<<"ill"<<endl<<endl<<endl<<endl<<endl<<endl<<endl<<endl;
            err_ = -1.0;
            return;
        }
...
void getJacobiErr_l(const Vector2d p1,const Vector2d p2,const Vector3d l_proj,Vector2d& err,Matrix23d& jacobiErr_l)
{
    double l1=l_proj(0);
    double l2=l_proj(1);
    double ln2=l1*l1+l2*l2;
    double ln=sqrt(ln2);
    double e1=l_proj(0)*p1(0)+l_proj(1)*p1(1)+l_proj(2);
    double e2=l_proj(0)*p2(0)+l_proj(1)*p2(1)+l_proj(2);
    jacobiErr_l(0,0)=p1(0)-l1*e1/ln2;
    jacobiErr_l(0,1)=p1(1)-l2*e1/ln2;
    jacobiErr_l(0,2)=1;
    jacobiErr_l(1,0)=p2(0)-l1*e2/ln2;
    jacobiErr_l(1,1)=p2(1)-l2*e2/ln2;
    jacobiErr_l(1,2)=1;
    jacobiErr_l=jacobiErr_l/ln;
...
void StereoFrameHandler:: optimizeFunctions(Matrix4d DT, Matrix6d &H, Vector6d &g, double &e )
{
    ...
    for( auto it = matched_ls.begin(); it!=matched_ls.end(); it++)
    {
        if( (*it)->inlier )
        {
            Plucker plucCam_=adjoint_se3(DT)*(*it)->plucCam;
            Vector3d l_proj=Config::getLineK()*plucCam_.head(3);
            Vector2d err_i;
            Matrix23d jacobiErr_l;
            getJacobiErr_l((*it)->spl_obs,(*it)->epl_obs,l_proj,err_i,jacobiErr_l);
            Matrix36d jacobil_Lc=Matrix36d::Zero();
            jacobil_Lc.block(0,0,3,3)=Config::getLineK();
            Matrix6d jacobiLc_x;
            jacobiLc_x=-adjoint_se3(plucCam_);
            
            Matrix26d J_aux;
            // jacobian
            J_aux = jacobiErr_l*jacobil_Lc*jacobiLc_x;
...
//                 w *= overlap;
            ...
            // update hessian, gradient, and error
            H_l += J_aux.transpose() *J_aux * w;
            g_l += J_aux.transpose() * err_i * w;
            e_l += r * r * w;
            N_l++;
        }
    }
