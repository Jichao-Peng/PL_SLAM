...
public:
    ...
    // SLAM parameters
    static int&     maxNumFrameLoss()    { return getInstance().max_num_frame_loss; }
    ...
    static double&  maxRatio12P()        { return getInstance().max_ratio_12_p; }
    //
    static int&    maxPointMatchNum()    { return getInstance().max_point_match_num; }
    static double& pointMatchRadius()    { return getInstance().point_match_radius; }
    ...
    static int&     lsdOctaveNum()       { return getInstance().lsd_octave_num; }
    ...
    //
    static int&     maxLineMatchNum()    { return getInstance().max_line_match_num; }
    static double&  lineMatchRadius()    { return getInstance().line_match_radius; }

    // EXPERIMENTAL line cuting / weighting
//    static bool&    useLineWeight()      { return getInstance().use_line_weight; }
    ...
    static bool&    cutWithMaxVol()   { return getInstance().max_vol_line_cut; }
    ...
    //
    static double&  motionStepThres()    { return getInstance().motion_step_th; }

private:

    // SLAM parameters
    int    max_num_frame_loss;
    ...
    double point_match_radius;
    double line_match_radius;

    int    max_point_match_num;
    ...
    int    max_line_match_num;

    // line cuting / weighting
//    bool   use_line_weight;
    bool   use_line_conf_cut;
    double ratio_disp_std;
    double ratio_disp_std_hor;
    bool   max_vol_line_cut;
